...
void Game::Update() {
  ...
  if (patternSpawnTimer <= 0) {
    ...
    switch (pattern) {
      case 0:
        Enemy::SpawnFlower(pos, bullets);
        break;
      case 1:
        Enemy::SpawnLine(pos, bullets);
        break;
      case 2:
        Enemy::SpawnRandom(pos, bullets);
        break;
    }
  }
...
void Game::CheckCollisions() {
  ...
  for (auto& b : bullets) {
    ...
    if (dx * dx + dy * dy <
        (b.radius + player.radius) * (b.radius + player.radius)) {
      b.active = false;
  ...
  for (auto& e : enemies) {
    ...
    if (dx * dx + dy * dy <
        (e.radius + player.radius) * (e.radius + player.radius)) {
      e.active = false;
      player.health--;
    }
  }
