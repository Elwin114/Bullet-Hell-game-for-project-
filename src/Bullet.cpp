...
Bullet::Bullet(Vector2 position, Vector2 velocity, float r, bool enemy,
               int satelliteCount, float orbitR, float orbitS)
    : pos(position),
      vel(velocity),
      radius(r),
      active(true),
  ...
  for (int i = 0; i < satelliteCount; ++i) {
    ...
    Vector2 offset = {cosf(initAngle) * orbitRadius,
                      sinf(initAngle) * orbitRadius};
...
void Bullet::Update() {
  ...
  if (type == VORTEX) {
    ...
    for (size_t i = 0; i < satellites.size(); ++i) {
      ...
      Vector2 offset = {cosf(baseAngle) * orbitRadius,
                        sinf(baseAngle) * orbitRadius};
  ...
  if (pos.x < -50 || pos.x > GetScreenWidth() + 50 || pos.y < -50 ||
      pos.y > GetScreenHeight() + 50) {
    active = false;
